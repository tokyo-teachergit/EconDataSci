...
#Load a data file
df = pd.read_csv("jgbcme_all.csv", #Read CSV file into df = DataFrame. Bank Of Japan data set is here. https://www.mof.go.jp/english/policy/jgbs/reference/interest_rate/index.htm
		sep=",", 
		engine = "python", #to load Japanese CSV file.
		encoding = "utf-8", #If utf-8 doesn't work, use Shift-JIS, which is the standard text-coding of Japanese public instititions 
...
plt.savefig('jgb10yAll-sample.pdf')
plt.savefig('jgb10yAll-72.png', dpi=72)
plt.savefig('jgb10yAll-300.png', dpi=300)
plt.savefig('jgb10yAll-600.png', dpi=600) #Save files in multiple resolution at once.
plt.show()
