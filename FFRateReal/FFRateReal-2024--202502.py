...
#Load a data file
df1 = pd.read_csv("daily-treasury-rates-20250220.csv", #to read CSV file into df1 = DataFrame.
		sep=",", 
...
print(type(df1["Date"]))
print("\nThis is the column with header name 10 YR.\n")
print(df1["10 YR"])
print("\nThis is the data type of the column with header name ""10 YR"".\n")
print(type(df1["10 YR"]))
...
df2 = pd.read_csv("daily-treasury-rates-2024.csv", #to read CSV file into df2 = DataFrame.
		sep=",", 
		engine = "python", #to load Japanese CSV file.
		encoding = "utf-8",
        header=[0], #header=[1] = use the second row for comlumn headers, header=[0] = use the first row for comlumn headers. PANDAS count 0 for the first row, 1 for the second row.  
...
print("\nThe datatype of 10 YR is object and it is not suitable for plotting. Let's change the datatype of 10 YR from object to float.\n")
DATERANGE['10 YR'] = pd.to_numeric(DATERANGE['10 YR'])
print(DATERANGE.info())
...
print(FEDFUNDSDF.info())
print(FEDFUNDSDF)
FEDFUNDSDF = FEDFUNDSDF.loc['2022-01-01':'2025-02-01']
print(FEDFUNDSDF.info())
print(FEDFUNDSDF)
print("\nWell done, you finished the checking part.\n")
...
plt.plot(DATERANGE.index, DATERANGE["10 YR"], color ='green',
         marker ='o', markersize = 0.1, 
         label ='US 10 Year Real')
ax1.tick_params(axis='y', labelcolor='green')
#plt.plot(df1["Date"], df1["Date"], color ='g',
#         linestyle ='dashed', linewidth = 2,
#         label ='JGB10YearYield')
ax1.set_ylim(bottom=0, top=6, emit=True, auto=False, ymin=None, ymax=None) #Y-Axis
...
ax2.set_ylim(bottom=0, top=6, emit=True, auto=False, ymin=None, ymax=None) #Y-Axis
...
#Data Save Section
plt.savefig('FFRateReal-2024--202502.pdf')
plt.savefig('FFRateReal-2024--202502b.png', dpi=300)
plt.savefig('FFRateReal-2024--202502c.png', dpi=600) #Save files in multiple resolution at once.
plt.show()